...
<p align='center'>
    <a href='vuelite-live-editor.vercel.app'>
...
> **Note**: 다양한 사용 사례를 확인하고 직접 테스트 할 수 있는 [Playground](vuelite-live-editor.vercel.app)를 지원합니다.
...
<p align="center">
  ...
  <a href='vuelite-live-editor.vercel.app
' target="_blank">vuelite-live-editor.vercel.app
</a>
</p>
...
컴포넌트를 분리한다는 것은 각 컴포넌트의 옵션 객체를 독립적으로 정의하고, 이를 통해 컴포넌트가 개별적으로 동작하도록 하는 것을 의미합니다. 싱글 파일 컴포넌트`(SFC)` 포맷을 지원하지 않기 때문에, 옵션을 분리하면서도 마크업의 분리를 위해 `<template>` 태그를 사용하는 방식을 택하였습니다.

`DOM`과 `ViewModel`을 연결하여 양방향 바인딩을 구현하기 위해, 새로운 컴포넌트는 새로운 인스턴스를 필요로 합니다. 따라서 컴포넌트를 추가할 때는 독립적인 새로운 인스턴스를 만들어 관리해야 합니다. 최상위 컴포넌트를 제외한 나머지 컴포넌트들은 전역 또는 로컬 컴포넌트로 등록되며, 해당 컴포넌트가 마크업에서 사용될 때 옵션을 기반으로 새로운 인스턴스를 생성합니다. 이 과정에서 `<template>` 태그의 복사본이 해당 컴포넌트와 바인딩되어, 최종적으로 컴포넌트의 위치에 교체되어 동작하게 됩니다.
...
`Vue.js`의 `Composition API` 문법들을 부분적으로 제공합니다. 두 가지 `API` 스타일은 모두 동일한 시스템에 의해 동작하는 다른 인터페이스입니다. `Composition API`는 함수 범위에서 직접 반응형 데이터를 선언하는 방식으로 리액트의 함수형 컴포넌트에서 훅을 사용하는 것 과 사용성이 많이 유사하며, `Option API` 보다 확실히 자유로운 형식으로 유연한 로직을 구성할 수 있습니다.
