...
- 싱글 파일 컴포넌트 포맷을 지원하지 않기때문에 옵션에서 `styles`를 지원합니다.
...
- ***created*** :
모든 옵션을 전부 초기화되고, 데이터들에 `Reactivity`가 주입된 단계로 이 시점부터 데이터에 접근 가능한 상태  

- ***mounted*** :
인스턴스가 생성된 단계로 템플릿이 파싱되어 데이터와 매핑되고, `viewmodel`과 `DOM`이 연결되어 초기 렌더링이 완료된 상태

- ***beforeUpdate***
반응성을 주입한 데이터가 변화를 감지하고 `viewmodel`에 변경되었지만 아직 실제 `DOM`에 반영되지 않은 단계
